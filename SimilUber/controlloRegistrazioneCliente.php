...
<?php
	...
	{
    mysqli_close($conn); 
    echo "<a href='registrazioneCliente.php'> Email già presente nel database gia' utilizzato. Riprova. </a>";
	}
	else
	{				   					  
    $comando = "INSERT INTO utente(username, password, email, nome, cognome, data_nascita , citta_residenza, via_residenza, numero_civico, modalita_pagamento, numero_patente, feedback) 
				VALUES('".$username."', '".$password."', '".$email."', '".$nome."', '".$cognome."', '".$data_nascita."', '".$citta_residenza."', '".$via_residenza."', '".$numero_civico."','".$modalita_pagamento."', NULL , NULL);";
		echo $comando;
		if (!mysqli_query($conn, $comando)){
		  echo "Inserimento fallito <br />"; 
		echo "<a href='registrazioneCliente.php'> Si e' verificato un errore tecnico: prego riprovare. </a>";
		mysqli_close($conn);
		}else{
			mysqli_close($conn);
